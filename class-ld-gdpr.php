...
if ( ! class_exists( 'LearnDash_GDPR' ) ) {
	...
	class LearnDash_GDPR {
		...
		public function export_course_progress( string $email_address, int $page ): array {
			...
			/**
			 * Nikka: 
			 * This argument array ($courses_registered_query_args) is never
			 * used. At least not what I can see. 
			 */
			...
			/**
			 * Nikka: 
			 * I added this if statement. It’s needed because the WP_Query 
			 * includes all course IDs when an empty $course_progress_ids is 
			 * passed as post__in argument. 
			 */
			...
			foreach ( $course_progress as $course_id => $coursep ) {
				...
				/**
				 * Nikka:
				 * What is $completed supposed to do here? I cannot find any use
				 * for it. 
				 */
				...
				if ( ! empty( $coursep['lessons'] ) ) {
					foreach ( $coursep['lessons'] as $lesson_id => $status ) {
						...
						/**
						 * Nikka:
						 * Let’s add support for localization. We use different 
						 * strings for lessons and topics to make it compatible
						 * with Scandinavian langagues (and probably other 
						 * languages I don’t know),
						 */

						$status = ( true === (bool) $status ? esc_html_x( 'Completed', 'Completed lesson', 'learndash' ) : esc_html_x( 'Not completed', 'Not completed lesson', 'learndash' ) );
						
						/**
						 * Nikka:
						 * Let’s add this filter so that plugins with course
						 * timers can add the time spent. 
						 */

						$status = apply_filters( 'learndash_privacy_export_lesson_status', $status, $user->ID, $course_id, $lesson_id );
				...
				if ( ! empty( $course_progress[ $course_id ]['topics'] ) ) {
					foreach ( $course_progress[ $course_id ]['topics'] as $lesson_id ) {
						foreach ( $lesson_id as $topic_id => $status ) {
							 ...
							 * Let’s add support for localization. We use different 
							 * strings for lessons and topics to make it compatible
							 * with Scandinavian langagues (and probably more 
							 * languages I don’t know).
							 */

							$status = ( true === (bool) $status ? esc_html_x( 'Completed', 'Completed topic', 'learndash' ) : esc_html_x( 'Not completed', 'Not completed topic', 'learndash' ) );
							...
							$status = apply_filters( 'learndash_privacy_export_topic_status', $status, $user->ID, $course_id, $lesson_id, $topic_id );
				...
				/**
				 * Nikka:
				 * The course status in the personal data export wasn’t the 
				 * course status but rather the status of the course’s final 
				 * topic. Let’s change that to the course status and add a 
				 * filter as well. (I re-use variable names and write long lines 
				 * to keep the syntax consistent with the rest of the class.)
				 */

				$status = learndash_course_status( $course_id, $user->ID, true );
				$status = ( 'completed' === $status ? esc_html_x( 'Completed', 'Completed course', 'learndash' ) : esc_html_x( 'Not completed', 'Not completed course', 'learndash' ) );
				$status = apply_filters( 'learndash_privacy_export_course_status', $status, $user->ID, $course_id );
